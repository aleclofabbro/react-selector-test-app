...
const getSelected = (opts: [string, boolean][]) => opts.filter(([, sel]) => sel).map(([val]) => val)
...
export default function App() {
  ...
  const [singleOpts, setSingleOpts] = useState<[string, boolean][]>(() => randomOpts(5, 1))

  const [multiOpts, setMultiOpts] = useState<[string, boolean][]>(() => randomOpts(7, 3))
  const [selOpts, setSelOpts] = useState<[string, boolean][]>(() => randomOpts(7, 3))
  ...
  return (
    <div className="App">
      ...
      <br />
      <select multiple name="sel" value={form.values.sel} onChange={form.handleChange}>
        {selOpts.map(([key]) => (
